...
public class ColorSettingSwitcherComponent implements IGuiComponent {
    ...
    public ColorSettingSwitcherComponent(Setting<Color> setting, int x, int y) {
    ...
    public void onClick(int x, int y, int state) {
        if (GuiUtil.isHoveringOnTheComponent(this, x, y)) {
            if (state == 0) {
                ...
                setting.setValue(colorFromPoint(new Point2D.Double(x, y)));
            }
        }
    }
    ...
    public void draw() {
        ...
        Render2d.drawFilledCircle(pointFromColor(), setting.getValue(), 3);
    }
